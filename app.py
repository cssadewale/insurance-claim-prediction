...
#
# ── CONFIRMED from model.n_features_in_ = 13 ─────────────────────────────────
# The model was trained WITHOUT log_building_dimension and log_building_age.
# Those features exist in the notebook but were not part of the training X.
# Only 5 numerical features were scaled during training.
SCALER_MEANS = {
...
# Exactly 13 features — confirmed by model.n_features_in_ = 13
# log_building_dimension and log_building_age are excluded because
# the model was saved before those columns were added to the training X.
FEATURE_COLUMNS = [
    "insured_period",
    "building_dimension",
    "number_of_windows",
...
def engineer_features(
    ...
    Steps (must match what the model was actually trained on):
      1. Compute transformed_insured_period (reflected log1p)
      2. One-hot encode categorical features (drop_first=True, same reference groups)
      3. Standardise the 5 numerical features using training-set μ and σ
      4. Return the 13-feature ordered array

    Note: log_building_dimension and log_building_age are intentionally excluded
    because model.n_features_in_ = 13 confirms the model was trained without them.
    """

    # ── Step 1: Only the transformation the model actually received ───────────
    # reflected log1p: insured_period is left-skewed (peak at 1.0),
    # so (1 - x) flips it right, then log1p compresses the tail
    transformed_insured_period = np.log1p(1.0 - insured_period)

    # ── Step 2: One-hot encoding (same reference groups as pd.get_dummies, drop_first=True) ──
    # Reference groups (the category dropped for each feature):
    ...
    # ── Step 3: Assemble the 13-feature dictionary ────────────────────────────
    raw = {
    ...
    # ── Step 4: Standardise the 5 numerical features ──────────────────────────
    # SCALER_MEANS/STDS only contains the 5 features that were scaled during training
    for feat in SCALER_MEANS:
        raw[feat] = (raw[feat] - SCALER_MEANS[feat]) / SCALER_STDS[feat]

    # ── Step 5: Return ordered DataFrame (13 features, model-required order) ──
...
if page == "🔮 Predict Claim Risk":
    ...
    if submitted:
        with st.spinner("Running prediction pipeline..."):
            ...
            # This bypasses sklearn's feature-name check (which compares the
            # DataFrame column names to the names the model was trained with).
            # The column ORDER in FEATURE_COLUMNS already matches training order,
            # so positional matching is correct and safe.
            prediction    = int(model.predict(feature_vector.values)[0])
            claim_prob    = float(model.predict_proba(feature_vector.values)[0][1])
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="sec-head">🎯 Top Feature Importances (Gini Impurity Reduction)</div>', unsafe_allow_html=True)
    fi_df = pd.DataFrame({
        "Rank": range(1, 11),
        "Feature": [
            "building_dimension", "building_age", "number_of_windows",
            "transformed_insured_period", "insured_period",
            "building_type_2", "building_type_3", "building_painted_V",
            "residential_1", "settlement_U",
        ],
        "Importance": [0.3210, 0.2480, 0.1350, 0.0890, 0.0740,
                       0.0420, 0.0310, 0.0240, 0.0180, 0.0120],
        "Note": [
            "Strongest — claim buildings 2.2× larger",
            "Second — older = higher risk",
            "Third — 3× more windows in claim buildings",
            "Transformed insured period", "Original insured period",
            "Building type 2 vs 1", "Building type 3 vs 1",
            "Painted buildings signal",
            "Residential status", "Urban settlement",
        ],
    })
    st.dataframe(
    ...
    st.markdown("""
    - `building_dimension` is the **strongest single predictor** — claim buildings have a median 2.2× larger
    - `building_age` is the **second most important** — older buildings claim more frequently
    - `number_of_windows` is the **third signal** — claim buildings have 3× the median window count
    - **SHAP confirms:** High building dimension and high building age both push predictions toward Claim (1)

    > *Note: The model was trained on 13 features — `building_dimension`, `building_age` and their*
    > *original forms are the primary drivers. Log-transformed versions were computed in the notebook*
    > *but were not included in the final training X.*
    """)
