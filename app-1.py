...
# ── DIAGNOSTIC — shows what the model actually expects ──────────────────────
# This block is TEMPORARY — remove it once the feature mismatch is resolved.
# It exposes the model's internal feature requirements in the sidebar so we
# can align the app's input pipeline exactly to what was used during training.
with st.sidebar:
    st.markdown("---")
    st.markdown("### 🔧 Model Diagnostics")
    st.markdown(f"**Features expected:** `{model.n_features_in_}`")
    if hasattr(model, "feature_names_in_"):
        st.markdown("**Feature names (from training):**")
        for i, name in enumerate(model.feature_names_in_):
            st.markdown(f"&nbsp;&nbsp;`[{i:02d}]` {name}")
    else:
        st.markdown("*No feature names stored — model was trained on a numpy array.*")
    st.markdown("---")
